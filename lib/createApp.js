...
exports.run = (name) => {
  if (!name) {
    console.error("Usage: quasar create-app <name>");
    process.exit(1);
  }
  ...
  // server
  w(
    join(base, "server", "routes.ts"),
    `import t from "@rbxts/t";
import { defineRoutes, r } from "@quasar/router";
export = defineRoutes(ns => ns.namespace("${name}")
  .route("ping", r.get({ in: t.nil, out: t.interface({ ok: t.boolean }), handler: async () => ({ ok: true }) }))
);`
  );
  w(join(base, "server", "services", "index.ts"), "");
  w(join(base, "server", "config.ts"), "export const RATE = 5;\n");
...
export type TPingOut = { ok: boolean };`
  );
  w(join(base, "shared", "models.ts"), "export type Example = { id: string };\n");
