...
exports.run = () => {
  ...
  if (!fs.existsSync(cfgPath)) {
    writeJson(cfgPath, {
      appsDir: "src/apps",
      sentinelPath: "src/common/chisel/__generated__/manifest-hash.ts",
    });
  ...
  const globalSharedDir = path.join(cwd, "src", "global", "shared");
  fs.mkdirSync(path.join(globalSharedDir, "state"), { recursive: true });

    // enums.ts (shared so both client/server can import)
  const enumsFile = path.join(globalSharedDir, "enums.ts");
  if (!fs.existsSync(enumsFile)) {
    fs.writeFileSync(
      enumsFile,
      [
        "// Global enums available to all apps",
    ...
    console.log("✔ created src/global/shared/enums.ts");
  }
  ...
  if (!fs.existsSync(stateIndex)) {
    fs.writeFileSync(
      stateIndex,
      [
        "// Global state (shared types/selectors) — put pure/shared code here",
        "export type GlobalState = { phase: number; };",
        "export const initState = (): GlobalState => ({ phase: 0 });",
        ""
      ].join("\n")
    );
    console.log("✔ created src/global/shared/state/index.ts");
  }
  ...
  if (!fs.existsSync(globalServerDir)) {
    ...
    console.log("✔ created src/global/server");
  }

  // 3) sentinel import anchor
  const anchorDir = path.join(cwd, "src", "common", "chisel");
  ...
  if (!fs.existsSync(anchor)) {
    ...
    console.log("✔ created src/common/chisel/index.ts");
  }
  ...
  ensureLine(path.join(cwd, ".gitignore"), "src/common/chisel/__generated__/");
